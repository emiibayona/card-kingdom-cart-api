...
export default async function handler(req, res) {
  ...
  if (req.method === "OPTIONS") {
    ...
    res.setHeader(
      ...
      "Content-Type,Authorization,X-Requested-With,X-PROXY-KEY"
    );
  ...
  if (!isHostAllowed(target, allowedHosts)) {
    return res
      .status(403)
      .json({ error: "Host not allowed by proxy configuration" });
  }

  try {
    const fetchOptions = {
      method: req.method,
      headers: copySelectedHeaders(req.headers, [
        "accept",
        "content-type",
        "authorization",
        "x-requested-with",
    ...
    res.setHeader("Access-Control-Allow-Origin", "*");
    res.status(500).json({ error: "im can't" + err.message });
  }
}
